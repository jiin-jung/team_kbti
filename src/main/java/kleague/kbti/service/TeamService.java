...
public class TeamService {
    ...
    public String generateVibeCheck(TeamStats stats) {
        ...
        // 1. 전술 유형별 서두 (분석하신 KMeans 결과 반영)
        vibe.append(getTypeIntro(stats.getTypeId()));

        // 2. 공격 지표 분석 (Real_Goals_PerGame)
    ...
    /**
     * 분석 데이터를 바탕으로 임시 팀 데이터를 생성합니다 (DB 연동 전 테스트용)
     */
    public TeamStats getMockTeamStats(String teamName) {
        if (teamName.equals("대구FC")) {
            return TeamStats.builder()
                    .teamName("대구FC").goalsPerGame(1.18).passSuccessRate(83.8)
                    .clearancePerGame(26.67).shotOnTarget(4.15)
                    .tempo(99).directness(98).fight(76).typeId(1)
                    .build();
        }
        // 기본값 (강원FC 예시)
        return TeamStats.builder()
                .teamName("강원FC").goalsPerGame(1.61).passSuccessRate(87.2)
                .clearancePerGame(23.36).shotOnTarget(4.39)
                .tempo(55).directness(62).fight(40).typeId(2)
                .build();
    }
}